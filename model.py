...
class WaveNet:
	...
	def createModel(self, input):
		...
		outputs = []
		with tf.named_scope('Dilated Stack'):
			for layerNumber, dilation in enumerate(self.dilations):
				with tf.named_scope('Dilated Layer '+str(layerNumber)):
					output, currentLayer = self.createDilatedLayer(currentLayer)
					outputs.append(output)

		#post processing layer
		with tf.named_scope('Post-Processing'):
